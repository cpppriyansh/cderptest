...
const BlogsAdminPanel = () => {
    ...
    try {
      ...
      Object.keys(blogData).forEach((key) => {
        ...
        if (value !== null && value !== "" && value !== undefined) {
          if (key === "blogImage" && value instanceof File) {
          ...
          } else if (key === "blogImage") {  // Preserve existing image if no new file
            if (blogData.existingImageUrl) {  // Assuming CreateBlogPost passes existingImageUrl
              formData.append("existingImage", blogData.existingImageUrl);  // Or "image" if backend handles it
              if (blogData.existingImagePublicId) {
                formData.append("existingImagePublicId", blogData.existingImagePublicId);
              }
            }
          } else if (
            key !== "imagePreview" &&
            ...
            } else if (key === "keywords" && Array.isArray(value)) {  // New: Handle keywords similarly to tags
              formData.append("keywords", JSON.stringify(value));
            } else {
      ...
      let method = "POST";
      if (editingBlog && (blogData._id || editingBlog._id)) {  // Stricter check: fallback to editingBlog._id
        const id = blogData._id || editingBlog._id;  // Ensure ID is always used for updates
        if (!id) {
          throw new Error("Missing blog ID – cannot update!");  // Guard against accidental creates
        }
        url = `${API_BASE}/api/blogs/${id}`;
        method = "PUT";
      }
      ...
      console.error("[handleSaveBlog Error]:", error);  // Add logging for debugging
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleEdit = (blog) => {
    if (!blog._id) {
      showNotification("Invalid blog – missing ID!", "error");
      return;
    }
    setEditingBlog(blog);
