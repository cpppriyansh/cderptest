...
// Simplified Mobile Carousel Component without fixed container
const MobileCarousel = ({ images, position, direction }) => {
  const carouselRef = useRef(null);
  
  // Create enough duplicates for seamless infinite scroll
  const displayImages = [...images];
  if (position === 'bottom') {
    // For bottom carousel, reverse the images
    displayImages.reverse();
  }
  const duplicatedImages = [...displayImages, ...displayImages, ...displayImages, ...displayImages];
  const itemWidth = 90; // 80px width + 10px margin
  const totalWidth = duplicatedImages.length * itemWidth;
  ...
  useEffect(() => {
    ...
    const keyframeName = `scroll-${direction}-${position}`;
    
    // Create keyframes for continuous movement
    let keyframes;
    let initialTransform;
    
    if (direction === 'left') {
      // Top carousel: move from right to left
      ...
      keyframes = `
        @keyframes ${keyframeName} {
          ...
          100% {
            transform: translateX(-${totalWidth / 4}px);
          }
        }
      `;
    } else {
      // Bottom carousel: move from left to right
      initialTransform = `translateX(-${totalWidth / 4}px)`;
      keyframes = `
        @keyframes ${keyframeName} {
          0% {
            transform: translateX(-${totalWidth / 4}px);
    ...
    // Apply animation
    carousel.style.animation = `${keyframeName} ${animationDuration}s linear infinite`;
  ...
  }, [images, direction, position, totalWidth]);

  return (
    <div className={`relative w-full h-24 overflow-hidden ${position === 'top' ? '-mb-3' : '-mt-6'} block sm:hidden`}>
      {/* Moving Carousel - Full width now */}
        ...
        >
          {duplicatedImages.map((image, index) => (
            <div 
              ...
              className="flex-shrink-0 w-20 h-16 mr-2 relative"
            >
              <img 
                src={image.src} 
...
export default function Achievements({
  ...
  dragDampening = 2,
  openedImageWidth: propOpenedImageWidth,
  openedImageHeight: propOpenedImageHeight,
  ...
  const [imageDimensions, setImageDimensions] = useState({
    width: propOpenedImageWidth || (typeof window !== 'undefined' && window.innerWidth < 768 ? '300px' : '400px'),
    height: propOpenedImageHeight || (typeof window !== 'undefined' && window.innerWidth < 768 ? '300px' : '400px')
  });

  const openedImageWidth = imageDimensions.width;
  const openedImageHeight = imageDimensions.height;
  ...
  // Custom CSS for 3D transforms and complex calculations
  const cssStyles = `
    /* Core 3D Sphere Setup - Custom properties needed for calculations */
    .sphere-root {
      --radius: 520px;
      --viewer-pad: 72px;
    ...
    /* 3D Transform Styles - Cannot be replaced with Tailwind */
  ...
  return (
    <div className="relative">
      <BackgroundAnimation />
      <div className="w-full text-center mb-0 md:mb-1 p-6">
      <h2 className="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-bold mb-3 mt-4 md:mt-3 px-4" style={titleStyle}>
          Our Achievements
        </h2><div style={underlineStyle} />
      </div>
      
      <style dangerouslySetInnerHTML={{ __html: cssStyles }} />
      
      {/* Top Mobile Carousel - Moves LEFT continuously */}
      <MobileCarousel 
        images={images} 
        position="top" 
        direction="left"
      ...
      <div 
        ref={wrapperRef} 
        className="relative mx-auto w-full h-[380px] md:h-auto md:aspect-square max-w-[700px] md:max-w-[800px] rounded-none md:rounded-full overflow-hidden transition-transform duration-300 -mt-2 md:mt-0"
      >
        <div
          ref={rootRef}
          className="sphere-root relative w-full h-full box-border"
          style={{
            ['--segments-x']: segments,
            ['--segments-y']: segments,
        ...
        >
          ...
          >
            <div className="stage absolute inset-0 m-auto w-full h-full grid place-items-center">
              <div ref={sphereRef} className="sphere absolute">
                {items.map((it, i) => (
                  <div
                    key={`${it.x},${it.y},${i}`}
                    className="sphere-item absolute m-auto transition-transform duration-300"
                    data-src={it.src}
                    data-alt={it.alt}
                    data-offset-x={it.x}
                    data-offset-y={it.y}
                  ...
                  >
                    <div
                      className="item__image absolute inset-[10px] md:inset-[10px] block overflow-hidden cursor-pointer bg-gray-200 transition-transform duration-300 pointer-events-auto outline-none focus:outline-2 focus:outline-blue-500 focus:outline-offset-2 focus:z-[100] hover:scale-105 hover:z-50"
                      role="button"
                      tabIndex={0}
                    ...
                    >
                      <img
                        className="gallery-image w-full h-full object-cover pointer-events-none"
                        src={it.src}
                        draggable={false}
      ...
      {/* Bottom Mobile Carousel - Moves RIGHT continuously */}
      <MobileCarousel 
        images={images} 
        position="bottom" 
        direction="right"
